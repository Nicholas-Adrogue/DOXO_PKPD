...
n = length(maxDox);
muT = n*(n+1)/4;
sigT = sqrt(n*(n+1)*(2*n+1)/24);

diffMaxDox = abs(maxDox-(maxDoxCalc/1e3));
meanDiffMaxDox = mean(diffMaxDox); %ng/mL
sumDiffMaxDox = sum(diffMaxDox); %ng/mL
[MaxDoxPval,MaxDoxH,MaxDoxZt] = signrank(maxDox,(maxDoxCalc/1e3));
...
sumDiffWBCn = sum(diffWBCn); %#/mL
[MinWBCnPval,MinWBCnH,MinWBCnZt] = signrank(WBCn,(WBCnCalc/1e3/V_E/1e6));